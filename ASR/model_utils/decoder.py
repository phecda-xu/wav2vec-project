...
class BeamCTCDecoder(Decoder):
    def __init__(self, labels, lm_path=None, alpha=0, beta=0, cutoff_top_n=40, cutoff_prob=1.0, beam_width=100,
                 num_processes=4, blank_index=0, word_form="sinogram"):
    ...
    def convert_to_strings(self, out, seq_len):
        ...
        for b, batch in enumerate(out):
            ...
            for p, utt in enumerate(batch):
                size = seq_len[b][p]
                if size > 0 and (self.word_form == "sinogram" or self.word_form == "english"):
                    transcript = ''.join(map(lambda x: self.int_to_char[x.item()], utt[0:size]))
                elif size > 0 and self.word_form == "pinyin":
                    transcript = ' '.join(map(lambda x: self.int_to_char[x.item()], utt[0:size]))
                else:
                    transcript = ''
...
class GreedyDecoder(Decoder):
    def __init__(self, labels, blank_index=0, word_form="sinogram"):
        self.word_form = word_form
        ...
        for i in range(size):
            ...
            if char != self.int_to_char[self.blank_index]:
                ...
                if remove_repetitions and i != 0 and char == self.int_to_char[sequence[i - 1].item()]:
                    pass
                elif char == self.labels[self.space_index] and self.word_form == 'sinogram':
                    string += ''
                    offsets.append(i)
                elif self.word_form == "pinyin":
                    string += ' ' + char
                    offsets.append(i)
                else:
                    string = string + char
